...
func (a ArticleTag) GetByAID(db *gorm.DB) (ArticleTag, error) {
	...
	err := db.Where("article_id = ? AND is_deleted = ?", a.ArticleID, 0).First(&articleTag).Error
...
func (a ArticleTag) ListByTID(db *gorm.DB) ([]*ArticleTag, error) {
	...
	if err := db.Where("tag_id = ? AND is_deleted = ?", a.TagID, 0).Find(&articleTags).Error; err != nil {
		return nil, err
	}
...
func (a ArticleTag) ListByAIDs(db *gorm.DB, articleIDs []uint32) ([]*ArticleTag, error) {
	var articleTags []*ArticleTag
	err := db.Where("article_id IN (?) AND is_deleted = ?", articleIDs, 0).Find(&articleTags).Error
...
func (a ArticleTag) UpdateOne(db *gorm.DB, values interface{}) error {
	if err := db.Model(&a).Where("article_id = ? AND is_deleted = ?", a.ArticleID, 0).Limit(1).Updates(values).Error; err != nil {
		return err
	}
...
func (a ArticleTag) DeleteOne(db *gorm.DB) error {
	if err := db.Where("article_id = ? AND is_deleted = ?", a.ArticleID, 0).Delete(&a).Limit(1).Error; err != nil {
		return err
	}
